...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        for i in range(p, q):
            for j in range(m, n):
                kin = (cell[0]+i, cell[1]+j)
                if kin in self.mines:
                    count = count - 1
                elif kin not in self.mines | self.safes:
                    adding.add(kin)
    ...
    def make_random_move(self):
        ...
        for i in range(8):
            for j in range(8):
                if (i, j) not in self.mines | self.moves_made:
                    return (i, j)
        return None
        """
        ...
        # __ __ __
        # __ __ __
        # Making a random move(Improve random move)